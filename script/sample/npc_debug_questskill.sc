...
prontera.gat,152,187,4	script	スキル屋	779,{
	...
	next;
	if(@menu == 1) {
		switch(select("移動時HP回復","急所攻撃","オートバーサーク","チャージアタック","シュリンク")) {
		case 1:
			set '@skillid,144;
			close;
		case 2:
		...
		switch(select("エナジーコート","サイトブラスター","コンバーター製造","水属性エレメンタルチェンジ","地属性エレメンタルチェンジ","火属性エレメンタルチェンジ","風属性エレメンタルチェンジ")) {
		case 1:
			set '@skillid,157;
			break;
		case 2:
		...
		switch(select("矢作成","ファンタスミックアロー","チャージアロー","パンボイス","魅惑のウィンク")) {
		case 1:
			set '@skillid,147;
			break;
		case 2:
		...
		switch(select("ホーリーライト","レデムプティオ","気注入","気爆発")) {
		case 1:
			set '@skillid,156;
			break;
		case 2:
		...
		switch(select("石拾い","石投げ","砂まき","バックステップ","ソニックアクセラレーション","ベナムナイフ","クローズコンファイン")) {
		case 1:
			set '@skillid,151;
			break;
		case 2:
		...
		switch(select("ラウドボイス","チェンジカート","カートレボリューション","フェイクゼニー","貪欲")) {
		case 1:
			set '@skillid,155;
			break;
		case 2:
